...
def handler(job):
    ...
    if not script_json or not anchor_text or not anchor_wav_base64:
        return {
            "error": "Missing required parameters: script_json, anchor_text, or anchor_wav_base64"
        }
    ...
    with torch.inference_mode():
        for i, item in enumerate(script_json):
            ...
            temp = item.get("recommended_temperature", 0.75)

            torch.manual_seed(42 + i)
            if torch.cuda.is_available():
                torch.cuda.manual_seed_all(42 + i)

            # Single-text execution prevents tokenizer batch-padding tensor memory collision
